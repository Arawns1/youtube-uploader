...
async function uploadVideoToYouTube(video) {
  ...
  const description = `${video.description} - Clipe enviado por ${video.author}`
...
async function main() {
  try {
    const onMessage = async (content, properties) => {
      ...
      channel.sendToQueue(properties.replyTo, Buffer.from(youtubeLink), {
